...
const unknown = (symbol = 'dsh-cost-insights#JsonValue') => ({
  mode: 'strict',
  typeSymbol: symbol,
  schema: z.unknown(),
})
...
const invocation = (method, parameters = []) => ({
  ...
  parameters: parameters.map((name, index) => ({
    ...
    codec: {
      mode: 'strict',
      typeSymbol: `dsh-cost-insights#${method}Parameter${index}`,
      schema: parameterSchemas[name] || z.unknown(),
    },
  })),
  result: unknown(`dsh-cost-insights#${method}Result`),
...
export const TYPERT = {
  ...
  model: {
    ...
    face: 'host',
    services: [{
      description: 'Cost Insights ledger, pricing, balance and analytics service.',
      summary: 'Cost Insights host service.',
      tags: [],
